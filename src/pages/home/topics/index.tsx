...
import { getHotNodes, getRecentTopics, GetTopicsParam } from "guanggu-forum-api";
import './index.scss'
...
export default function Topics() {
  ...
  useEffect(() => {
    getHotNodes().then((nodes) => {
      setTabs(defaultTabs.concat(nodes.map((node) =>({
        title: node.title,
        type: 'default'
      }))));
    })
  ...
  useReady(() => {
   Taro.nextTick(() => {
     ...
     query.select(`#list_container`).boundingClientRect((res) => {
